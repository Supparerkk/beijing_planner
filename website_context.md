...
| `#day2` | วันที่ 2 · 18 ต.ค. | Lama Temple + Confucius Temple + ล่องเรือฉือชาไห่ + Yandaixie/หอกลอง | เหมือนด้านบน + `.attraction-callout` (ราคาค่าเช่าเรือ) | เนื้อหา hardcode | ระดับ 🟨 ปานกลาง — ย้ายมาจากวันพุธเดิม (แผน D) + เพิ่มล่องเรือแทน 798 และวาด `lm-svg-2` ใหม่ (2026-09-13) |
...
| `#day7` | วันที่ 7 · 23 ต.ค. | Summer Palace + 798 Art District + Sanlitun | เหมือนด้านบน | เนื้อหา hardcode | ระดับ 🟥 — ย้ายมาจากวันจันทร์เดิม (แผน D) + รับ 798/Sanlitun ต่อจากวันที่ 2 (2026-09-13) |
...
| 2026-09-13 | **วาด pixel-art วันที่ 2 ใหม่ให้ตรงกับแผนล่าสุด** — `lm-svg-2` เดิมเป็น "เจดีย์วัดลามะ + ปล่องไฟ 798" แต่ 798 ย้ายไปวันที่ 7 แล้ว จึงลบครึ่งขวา (ปล่องไฟ + อาคารโรงงาน 798) ออกแล้ววาดใหม่เป็น **ทะเลสาบฉือชาไห่: เรือถีบหลังคาสีทองพร้อมผู้โดยสาร 2 คน + เรือแจวลำเล็กด้านหลัง + หอกลอง (鼓楼) ฝั่งตรงข้าม + ผิวน้ำมีระลอกคลื่น** ส่วนเจดีย์วัดลามะครึ่งซ้ายคงเดิมไม่แตะต้อง | `index.html`: `<svg id="lm-svg-2">` (ลบ `<rect>` ที่ x ≥ 56 จำนวน 725 ตัว แล้วใส่ชุดใหม่ 720 ตัว) + comment กำกับเปลี่ยนเป็น "Day 2 SVG: Yonghe Temple Pagoda, Shichahai Boat &amp; Drum Tower" | ใช้เฉพาะสีที่มีอยู่แล้วในพาเลตของไฟล์ (`#be123c` `#0f172a` `#eab308` `#64748b` `#38bdf8` `#f8fafc` `#475569`) จึงเข้ากับภาพวันอื่น · viewBox ยังเป็น `0 0 120 24` เท่าเดิม · ไม่กระทบ `lm-svg` อื่นและไม่กระทบ `syncPixelArt()` | ตรวจไม่มี `<rect>` ซ้อนพิกัดเดียวกัน (0 duplicate cells), `<svg>` 11/11, `<div>` 890/890, `<article>` 10/10, render เป็น ASCII grid ตรวจด้วยตาแล้ว | Claude (AI) |
| 2026-09-13 | **เพิ่มล่องเรือฉือชาไห่ในวันที่ 2 และย้าย 798 + Sanlitun ไปบ่ายวันที่ 7** — วันที่ 2 (อา. 18 ต.ค.) เปลี่ยนช่วงบ่ายจาก "798 Art + Sanlitun" เป็น "ฉือชาไห่ (เช่าเรือ) → ตรอก Yandaixie/หอกลอง/สามล้อถีบหูท่ง → มื้อค่ำย่านหอกลอง-Houhai → กลับโรงแรม 19:15" พร้อม `.attraction-callout` ราคาค่าเช่าเรือ (เรือถีบ/ไฟฟ้าเริ่ม 180 ¥/ชม. · เรือแจว 80 ¥/คน/ครึ่ง ชม. · มัดจำ ~300-500 ¥ คืนเต็ม · เปิด 09:00-18:00 หยุดขายตั๋ว 17:00 · ไม่ต้องจอง สแกน QR ที่ท่าเหอฮวาซื่อฉ่าง · โทร 010-66125717) · วันที่ 7 (ศ. 23 ต.ค.) เปลี่ยนช่วงบ่ายจาก "หอกลอง/สามล้อถีบ/Nanluoguxiang/มื้อค่ำ Houhai" (ซ้ำกับวันที่ 2 แล้ว) เป็น "798 Art District 14:15-17:30 → Sanlitun มื้อค่ำ+เดินเล่น → กลับโรงแรม 21:15" · เหตุผล: 798 ต่อท้ายฉือชาไห่ไม่ได้เพราะแกลเลอรีปิด 18:00 และห่างกัน 50-60 นาที | `index.html`: timeline ของ `#day2` (แทนบล็อก 13:10-21:00 เดิม) และ `#day7` (แทนบล็อก 13:00-18:00 เดิม + เลื่อนเวลากลับเป็น 20:30-21:15), day-main-theme + tips-box + `.day-status-strip` ทั้ง 2 วัน, sidebar `.tab-name` 2 จุด, mobile nav emoji (day2 🛶 / day7 🎨), `pixelDayTitles` 2 บรรทัด, `.val-highlight-day` 8 จุด (ร้านย่าน Sanlitun/798 → วันที่ 7 · ร้านย่านหอกลอง/Houhai → วันที่ 2), เพิ่มแถว "เรือฉือชาไห่" ในตารางจองแบบยืดหยุ่นของแท็บ `#checklist` | ไม่กระทบวันอื่น · ไม่มีย่านซ้ำกันในทริปอีกต่อไป (ฉือชาไห่/หอกลองอยู่วันที่ 2 เท่านั้น · 798/Sanlitun อยู่วันที่ 7 เท่านั้น) · วันที่ 7 ยาวขึ้นจนถึง ~21:15 (แลกกับการไม่ตัด 798 ทิ้ง) · pixel-art `lm-svg-2` ถูกวาดใหม่แล้วในรอบถัดมาของวันเดียวกัน (ดูแถวบน) · `lm-svg-7` เป็นพระราชวังฤดูร้อนซึ่งยังตรงกับวันที่ 7 · **แก้บั๊กเดิมที่ค้างมาก่อนหน้านี้: `#day2` ขาด `</div>` ปิด `.timeline` 1 ตัว ทำให้ tag `<div>` ไม่สมดุล ตอนนี้ balance 890/890 แล้ว** | `node --check` ผ่าน · `<div>` 890/890, `<article>` 10/10, `<tr>` 20/20, `<td>` 60/60 · ตรวจลำดับเวลาในวันที่ 2 และ 7 ต่อเนื่องไม่ทับซ้อน | Claude (AI) |
